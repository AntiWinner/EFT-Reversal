...
public abstract class ParticleForceField : MonoBehaviour
{
	...
	protected virtual void LateUpdate()
	{
		...
		_E005 = base.transform.position + center;
		...
		for (int k = 0; k < _E008; k++)
		{
			...
			if (simulationSpace == ParticleSystemSimulationSpace.World)
			{
				for (int l = 0; l < particles; l++)
				{
					...
					parameters.scaledDirectionToForceFieldCenter.x = _E005.x - parameters.particlePosition.x;
					parameters.scaledDirectionToForceFieldCenter.y = _E005.y - parameters.particlePosition.y;
					parameters.scaledDirectionToForceFieldCenter.z = _E005.z - parameters.particlePosition.z;
					parameters.distanceToForceFieldCenterSqr = parameters.scaledDirectionToForceFieldCenter.sqrMagnitude;
					if (parameters.distanceToForceFieldCenterSqr < _E003)
					{
						float time = parameters.distanceToForceFieldCenterSqr / _E003;
			...
			else
			{
				...
				for (int m = 0; m < particles; m++)
				{
					...
					float time2 = parameters.distanceToForceFieldCenterSqr / _E003;
